...
* resets all vernalisation gene expressions and loads cultivar-specific parameters from `FLNparams`. It also sets the initial Haun stage to 0,
...
#### Calculate daily Haun Stage changes {#haun-stage-calculation}

If the crop has not emerged (i.e. before emergence), the Haun Stage delta ($\Delta HS$) is estimated based on thermal time ($TT$) and a base phyllochron:

$$
\Delta HS = \frac{TT}{\text{BasePhyllochron}} \cdot \text{EmergeDurationFactor}
$$

Where:

- If accumulated $TT > 90$:

$$
\text{EmergeDurationFactor} = e^{-0.015 \cdot (TT - 90)}
$$

Otherwise, if the crop **has emerged**, $\Delta HS$ is calculated from the model variable `deltaHaunStage`.

# Vernalisation Expression
...
## Vegetative vs Reproductive Phase

- If not yet vernalised:

$$
dBaseVrn = \text{BaseDVrnVeg} \cdot \Delta HS
$$

$$
dMaxVrn = \text{MaxDVrnVeg} \cdot \Delta HS
$$

$$
PpVrn3Fact = \text{PpVrn3FactVeg}
$$

- If vernalised but not reproductive:

$$
dBaseVrn = \text{BaseDVrnER} \cdot \Delta HS
$$

$$
dMaxVrn = \text{MaxDVrnER} \cdot \Delta HS
$$

$$
PpVrn3Fact = \text{PpVrn3FactER}
$$

Base and Max vernalisation levels are incremented:

$$
\text{BaseVrn} += dBaseVrn
$$
