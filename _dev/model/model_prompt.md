...
1. Locate the APSIM NG source code file: _apsimx/*/<ModelName>.cs
2. Open <ModelName>.cs and use its content as the primary source for generating science documentation.

File Handling:

- Use the template `_template/model_template.qmd` to structure the documentation.
- Output a QMD file: <ModelName>.qmd
- If <ModelName>.qmd does not exist:
    - Create a new QMD file under /docs/ using the same folder structure as the source code in _apsimx.
