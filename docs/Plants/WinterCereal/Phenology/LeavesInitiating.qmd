...
> Note: The [hourly interpolation](/docs/Models/Functions/HourlySinPpAdjusted.qmd) of temperature is different with [three hourly interpolation method](/docs/Models/Functions/ThreeHourAirTemperature.qmd) which is used in the [Thermal Time](/docs/Plants/Wheat/Phenology/ThermalTime.qmd).
...
We demonstrate progress of leaf initiation phase using [example simulations](/docs/Plants/Wheat/Examples/wheat_example1.qmd) for wheat. These simulations illustrate how Vrn expression progresses the development of wheat phenology.
...
```
```{r read-data}
file <- "docs/Plants/Wheat/Examples/wheat_example1.apsimx"
    
df <- read_example(file, max_stage = 5)
```

The [vernalisation rate parameters](/docs/Models/PMF/Phenology/CAMP/CultivarRateParams.qmd) are calculated for each cultivar using the [calcCAMPVrnRates](/docs/Models/PMF/Phenology/CAMP/CalcCAMPVrnRates.qmd) function at [Sowing](/docs/Models/Events.qmd#PlantSowing), and should be genotype-specific values. The following table shows the calculated parameters for Emu_Rock and Sunlamb cultivars:

```{r cultivar-rate-parameters, results = "asis"}

df |>
    dplyr::group_by(Cultivar) |>
    dplyr::slice(1) |> 
    dplyr::select(
        Wheat.Phenology.CAMP.Params.BaseDVrnER,
        Wheat.Phenology.CAMP.Params.BaseDVrnVeg,
        Wheat.Phenology.CAMP.Params.MaxDVrnER,
        Wheat.Phenology.CAMP.Params.MaxDVrnVeg,
        Wheat.Phenology.CAMP.Params.PpVrn3FactER,
        Wheat.Phenology.CAMP.Params.PpVrn3FactVeg,
        Wheat.Phenology.CAMP.Params.MaxVrn2,
        Wheat.Phenology.CAMP.Params.MethalationThreshold,
        Wheat.Phenology.CAMP.Params.ColdVrn1Fact
    ) |>
    tidyr::pivot_longer(cols = starts_with("Wheat.Phenology.CAMP.Params."), 
        names_to = "Parameter", 
        values_to = "Value") |>
    dplyr::mutate(Parameter = gsub("Wheat\\.Phenology\\.CAMP\\.Params\\.", "", Parameter)) |>
    dplyr::ungroup() |>
    dplyr::mutate(Value = as.numeric(Value)) |> 
    tidyr::pivot_wider(names_from = Cultivar, values_from = Value) |> 
...
```{r fig-fln}
plot_output(file, y = "Wheat.Phenology.CAMP.FLN",
    max_stage = 5.2) +
    ggplot2::scale_y_continuous(breaks = seq(3, 20, by = 2))
```
