...
The duration from sowing to emergence is first expressed in phyllochrons ($P_\text{emergence}$) using the base phyllochron value ($P_\text{base}$):
...
- $P_\text{base}$ is the base phyllochron duration.

### Base phyllochron duration of vernalisation treatment {#vern-phyllochron}

The base phyllochron duration of the vernalisation treatment ($P_\text{vern}$) is calculated as:
$$
P_\text{vern} = \frac{T_\text{vern} \times D_\text{V}}{P_\text{base}}
$$
...
- 2.85 is the assumed number of leaves before the terminal spikelet forms,
- 1.1 converts final leaf number to Haun Stage.
...
$$
\Delta V_{\text{base, veg}} = \frac{1}{\text{VS}_{SN} + P_{\text{emergence}}}
$$

Where:

- $\text{VS}_{SN}$ is the phyllochron at vernalisation saturation for the SN treatment,
- $P_{\text{emergence}}$ is the phyllochron duration from sowing to emergence.
...
$$
\Delta V_{\text{max, veg}} = \frac{1}{\text{VS}_{SV} + P_{\text{emergence}}}
$$
...
$$
V_{\text{max, Vrn2}} = \left( \text{EndVrn2}_{\text{LN}} + P_{\text{emergence}} \right) \times \Delta V_{\text{base, veg}}
$$
...
$$
V_{\text{base, LV}} = (P_\text{emergence} + VS_{\text{LV}}) \times \Delta V_{\text{base, veg}}
$$
...
$$
H_{\text{vern, LV}} = \min(P_\text{vern},\ P_\text{emergence} + VS_{\text{LV}})
$$

Where:
- $P_\text{vern}$ is the phyllochron duration of the vernalisation treatment,
- $P_\text{emergence}$ is the phyllochron duration from sowing to emergence,
- $VS_{\text{LV}}$ is the phyllochron at vernalisation saturation for the LV treatment.
...
The cold upregulation factor ($F_{\text{cold}}$, `ColdVrn1Fact` in [CultivarRateParams](CultivarRateParams.qmd)) is calculated as the ratio of cold-enhanced Vrn1 expression to baseline expression:

$$
F_{\text{cold}} = \frac{\Delta V_{\text{cold, max}}}{\Delta V_{\text{base, veg}}}
$$
