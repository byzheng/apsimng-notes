...
|-----------|-------|-----------------|------------------------------------------------------|
| MetData | [IWeather](/docs/Models/Interfaces/IWeather.qmd) | First Available | Provides daily maximum and minimum air temperatures from weather data. |
| SubDailyInterpolation | [SubDailyInterpolation](/docs/Models/Functions/SubDailyInterpolation.qmd) | Parent | Parent model that utilizes this interpolation method. |
...
`ThreeHourAirTemperature` only can be added as a child of a [SubDailyInterpolation](SubDailyInterpolation) node in the model tree. Right-click the parent node, select "Add Model...", and search for `ThreeHourAirTemperature` in the Filter Box.
