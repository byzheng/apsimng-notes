...
- response: optional list of named response curves for different phases (e.g., Pre-Emergence, Post-Emergence)
Usage in parent QMD:
...
if (!exists("params")) {
    params <- list(
        plant = "Oats",
        response = list(
            list(name = "Pre-Emergence", x = c(0, 30, 40), y = c(0, 30, 0)),
            list(name = "Post-Emergence", x = c(0, 30, 40), y = c(0, 30, 30))
        ),
        three_cardinal = FALSE,
        reference = ""
    )
...
setup_qmd()
if (is.null(params$response)) {
    stopifnot(length(params$response.x) == length(params$response.y))
    response <- list("Life Cycle" = list(name = "Life Cycle", x = params$response.x, y = params$response.y))
} else {
...
- Plant development rate increases linearly with temperature between base and optimum temperatures
- Development ceases below the base temperature and above a maximum temperature
...
```{r tbl-cardinal-temperatures, echo = FALSE, results = "asis"}
# Convert response list to table format
...
knitr::kable(
    response_df,
    col.names = c("Phase", "Temperature (°C)", "Thermal Time (°Cd)"),
    align = c("l", "c", "c")
)
```
...
```{r fig-thermal-time, fig.height = 3, fig.cap="Thermal Time Function with Three Cardinal Temperatures"}

plot_cardinal_temperature(response_df)
```
...
```{r data-example-response}
response_example <- response[[length(response)]] # Use the last response curve for example
response_x <- response_example$x
response_y <- response_example$y
```
For example, at the phase `r response_example$name`, the three hourly thermal time values would be calculated as follows:
