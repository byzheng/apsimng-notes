...
|-----------|-------|-----------------|------------------------------------------------------|
| CAMP | [CAMP](/docs/Models/PMF/Phenology/CAMP/CAMP.qmd) | Link, Ancestor | Parent phenology model providing helper methods and constants. |
| basePhyllochron | [IFunction](/docs/APSIM.Core/IFunction.qmd) | Link, Child By Name | Calculates the base phyllochron (thermal time per leaf). |
...
- `camp`: The ancestor [CAMP](/docs/Models/PMF/Phenology/CAMP/CAMP.qmd) model, which provides helper methods and constants

### Calculation Method

The `CalcCAMPVrnRates` model estimates [vernalisation rate parameters](CultivarRateParams.qmd) using:
...
`CalcCAMPVrnRates` can be added as a child of a [CAMP](/docs/Models/PMF/Phenology/CAMP/CAMP.qmd) node in the model tree. Right-click the CAMP node, select "Add Model...", and search for `CalcCAMPVrnRates` in the Filter Box.
