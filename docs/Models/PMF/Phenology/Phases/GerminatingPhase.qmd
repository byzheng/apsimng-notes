...
|----------------|---------------|------------------------------------------------------------------------------------------|
| `PlantSowing`  | Plant model   | Indicates when the plant is sown, allowing the phase to determine the soil layer depth where seeds are placed. This is essential for checking soil conditions relevant to germination. |
...
|----------------------|---------|-----------------------------------------------------------------------------|-------------|
| `Start`              | string  | Phenological stage at which this phase begins (e.g. `Sowing`).              | No          |
| `End`                | string  | Phenological stage that marks completion of the phase (e.g. `Germinated`).  | No          |
| `MinSoilTemperature` | double  | Minimum soil temperature required for germination (default is 0°C).         | No          |
| `GerminationDate`    | string  | Optional fixed calendar date to override environmental conditions.          | Yes         |
| `IsEmerged`          | bool    | Boolean flag indicating whether emergence has occurred. This property is available but not used in the germination logic. | Yes         |
