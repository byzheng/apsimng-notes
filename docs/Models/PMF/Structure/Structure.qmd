...
|----------------------|-----------------|------------------|------------------------------------------------------|
| [Plant](/docs/Models/PMF/Plant.qmd) | **First Available** | Broader plant context | Provides structural and lifecycle information for the plant. |
| [ILeaf](/docs/Models/Interfaces/ILeaf.qmd) | **First Available** | Organ-level interaction | Provides information on leaf cohort status, appearance, and expansion. |
| [Phenology](/docs/Models/PMF/Phenology/Phenology.qmd) | **First Available** | Developmental driver | Supplies current growth stage and timing of key developmental events. |
| [thermalTime](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Internal driver | Calculates accumulated temperature used for leaf and stem development. |
| [phyllochron](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Development timing | Thermal time interval between successive leaf appearances. |
| [finalLeafNumber](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Architecture limit | Maximum number of leaves produced by the main stem. |
| [heightModel](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Structural dimension | Function describing plant height progression with development. |
| [branchingRate](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Architecture complexity | Rate of branch initiation relative to main stem development. |
| [branchMortality](/docs/APSIM.Core/IFunction.qmd) | **Child Connection** | Stress response | Rate of branch mortality under environmental stress conditions. |
...
When the phenological stage reaches `LeafInitialisationStage` (Emergence in default), the Structure model performs leaf cohort initialization.
...
$$N_{tips}(t) = 1 + \sum_{i=emergence+1}^{t} \Delta N_{tip}$$
...
Leaf cohort(s) are initialised when no leaf exists (e.g. at emergence or defoliation). The number of cohorts initialised is determined by the `CohortsAtInitialisation` property of the connected ILeaf model. `CohortToInitialise` property is set as `CohortsAtInitialisation` property.

The number of appeared leaf cohorts or tips at initialisation is determined by the `TipsAtEmergence` property of the connected ILeaf model. At each leaf tip at emergence, a new leaf tip is [appeared](#leaf-tip-appearance). Then the `PotLeafTipsAppeared` and `CohortToInitialise` properties are incremented by 1, and the `AddLeafCohort` event with parameter of current leaf rank is raised.

In case of wheat, `CohortsAtInitialisation` is 2 and `TipsAtEmergence` is 1 by default. Therefore, at emergence, three leaf cohorts will be initialised and one leaf tip will appear. 

## Leaf Tip Appearance {#leaf-tip-appearance}

At each leaf tip appearance, the `TipToAppear` property is incremented by 1, and the `LeafTipAppearance` event is raised with parameter current total stem population (`TotalStemPopn`), cohort age (i.e. 0), final fraction (i.e. `NextLeafProportion`, fraction for final leaf and 1 for other leaves).
