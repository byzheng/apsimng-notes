...
```{r day-length, fig.height = 4, fig.cap="Day Length Variation Across Australia" }
...
[Photoperiod](/docs/Models/Functions/PhotoperiodFunction.qmd), or day length, impacts on ([phyllochron](/docs/Crops/Wheat/Phenology/Phyllochron.qmd)) when day length is shorter than 12 hours. 
...
The calculated photoperiod effect $f_d(d)$ is ranged from `r min(grid$f_d)` to `r max(grid$f_d)` mainly in winter and southern regions.

```{r fig-photoperiod-effect, fig.height = 5, fig.cap="Photoperiod Effect on Phyllochron"}
...
```{r fig-photoperiod-effect-city, fig.height = 5, fig.cap="Photoperiod Effect on Phyllochron for Capital Cities"}
