...
```{r echo=TRUE}
# This forces Quarto to treat file as dynamic
print("Quarto Preview. Delete it later")
```
...
- $V_{\text{base}}$: Baseline vernalisation expression, which is the cumulative amount of Vrn1 expressed under baseline conditions,
- $V_{\text{max}}$: Maximum vernalisation expression, which is the cumulative amount of Vrn1 expressed under maximum conditions (e.g. cold temperatures),
...
$$
\Delta V_{\text{base}} = \Delta V_{\text{base, veg}} \times \Delta HS
$$

Where:

- $\Delta V_{\text{base, veg}}$ is the baseline rate of Vrn1 expression during the vegetative phase, which is derived from the [calcCAMPVrnRates](CalcCAMPVrnRates.qmd) submodule at event [PlantSowing](/docs/Models/Events.qmd#PlantSowing),
- $\Delta HS$ is the daily Haun stage increment.

$$
\Delta V_{\text{max}} = \Delta V_{\text{max, veg}} \times \Delta HS
$$

Where:

- $\Delta V_{\text{max, veg}}$ is the maximum rate of Vrn1 expression during the vegetative phase, also derived from the [calcCAMPVrnRates](CalcCAMPVrnRates.qmd) submodule at event [PlantSowing](/docs/Models/Events.qmd#PlantSowing),
- $\Delta HS$ is the daily Haun stage increment.
...
$$
\Delta V_{\text{base}} = \Delta V_{\text{base, ER}} \times \Delta HS
$$

Where:

- $\Delta V_{\text{base, ER}}$ is the baseline rate of Vrn1 expression during the early reproductive phase, derived from the [calcCAMPVrnRates](CalcCAMPVrnRates.qmd) submodule at event [PlantSowing](/docs/Models/Events.qmd#PlantSowing),
- $\Delta HS$ is the daily Haun stage increment.

$$
\Delta V_{\text{max}} = \Delta V_{\text{max, ER}} \times \Delta HS
$$

Where:

- $\Delta V_{\text{max, ER}}$ is the maximum rate of Vrn1 expression during the early reproductive phase, derived from the [calcCAMPVrnRates](CalcCAMPVrnRates.qmd) submodule at event [PlantSowing](/docs/Models/Events.qmd#PlantSowing),
- $\Delta HS$ is the daily Haun stage increment.
...
$V_{\text{base}}$ and $V_{\text{max}}$ are accumulated since `isImbibed` is true, in general starting from germination.

$$
V_{\text{base}} = \sum_{t = \text{imbibed}}^{\text{today}} \Delta V_{\text{base}}
$$

$$
V_{\text{max}} = \sum_{t = \text{imbibed}}^{\text{today}} \Delta V_{\text{max}}
$$
...
$$
dCold = \text{RelCold} \cdot dBaseVrn \cdot \text{ColdVrn1Fact}
$$

If cold exposure surpasses a methylation threshold $\theta$:

$$
dVrn1 = \max\left(0, (Cold - \theta) - (Cold_{\text{yesterday}} - \theta)\right)
$$
...
Photoperiod gene expression is active only **after emergence**.

- Relative photoperiod ($RelPp$) is derived from `PpResponse`, scaled between 0 and 1.
- If $RelPp = 0$, then $MaxVrn2$ is reduced:

$$
MaxVrn2 = \max(0, MaxVrn2 - dBaseVrn)
$$

- If not yet vernalised:

$$
Vrn2 = MaxVrn2 \cdot RelPp
$$

- If $BaseVrn + Vrn1 > Vrn2$, expression of Vrn3 begins:

$$
dVrn3 = (PpVrn3Fact - 1) \cdot RelPp \cdot dBaseVrn
$$
...
Daily total vernalisation ($Vrn$) is computed:

- If not vernalised:

$$
Vrn = \min(MaxVrn, \max(0, BaseVrn + Vrn1 + Vrn3 - Vrn2))
$$
...
$$
dVrn = Vrn - Vrn_{\text{yesterday}}
$$

# Phase Transitions

- **Vernalisation complete** when:

$$
\text{isVernalised} = (Vrn \geq VSThreshold)
$$

- **Reproductive phase** begins when:

$$
\text{isReproductive} = (Vrn \geq TSThreshold)
$$

# Final Leaf Number (FLN)

FLN is estimated from the thermal switch Haun Stage ($TSHS$):

$$
FLN = \text{IntFLNvsTSHS} + 1.1 \cdot TSHS
$$
...
|--------|-------------|
| $TT$ | Thermal time |
| $\Delta HS$ | Daily Haun stage increment |
| $BaseVrn$, $MaxVrn$ | Cumulative vernalisation levels |
| $Vrn1$, $Vrn2$, $Vrn3$ | Vernalisation gene expressions |
| $RelPp$ | Relative photoperiod |
| $Vrn$ | Total vernalisation signal |
| $FLN$ | Final leaf number |
| $VSThreshold$, $TSThreshold$ | Thresholds for phase change |
