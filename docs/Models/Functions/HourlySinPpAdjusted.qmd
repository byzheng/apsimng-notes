...
* [IInterpolationMethod](/docs/Models/Interfaces/IInterpolationMethod.qmd)
...
|-----------|-------|-----------------|------------------------------------------------------|
| MetData | [IWeather](/docs/Models/Interfaces/IWeather.qmd) | Link | Provides daily minimum and maximum temperatures, and calculates day length, sunrise, and sunset times. |
| CE | [ControlledEnvironment](/docs/Models/Climate/ControlledEnvironment.qmd) | Link, Optional | If present, overrides the interpolated sub-daily values using provided hourly temperature data (`SubDailyTemperature`). This is used in controlled environment simulations (e.g., growth chambers) where actual hourly measurements are available. |
...
| `Tsset` | double | Temperature at the most recent sunset (°C) |
...
`HourlySinPpAdjusted` can be added as a child of a [SubDailyInterpolation](SubDailyInterpolation) node in the model tree. Right-click the parent node, select "Add Model...", and search for `HourlySinPpAdjusted` in the Filter Box.
...
vlines <- data.frame(
    Hour = c(sunrise, sunset),
    Label = c("Sunrise", "Sunset")
)

# Assign period for each hour
df$Period <- factor(
    ifelse(df$Hour < sunrise, "A: Midnight-Sunrise",
    ifelse(df$Hour < 12 + P, "B: Sunrise-Tmax",
    ifelse(df$Hour < sunset, "C: Tmax-Sunset", "D: Sunset-Midnight"))),
    levels = c("A: Midnight-Sunrise", "B: Sunrise-Tmax", "C: Tmax-Sunset", "D: Sunset-Midnight")
)

# Define colors for each period
period_colors <- c(
    "A: Midnight-Sunrise" = "#1b9e77",   # Teal
    "B: Sunrise-Tmax" = "#e7298a",       # Magenta
    "C: Tmax-Sunset" = "#7570b3",        # Indigo
    "D: Sunset-Midnight" = "black"     # Black
)

ggplot(df, aes(x = Hour, y = Temperature)) +
    geom_line() +
    geom_point(aes(color = Period, shape = Period), size = 3) +
    geom_vline(data = vlines, aes(xintercept = Hour, linetype = Label), color = "steelblue", size = 1, show.legend = FALSE) +
...
* **Source code:** [HourlySinPpAdjusted.cs on GitHub](http://github.com/APSIMInitiative/ApsimX/blob/master/Models/Functions/HourlySinPpAdjusted.cs)
