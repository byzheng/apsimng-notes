...
The [Phyllochron](Phyllochron.qmd) is the interval between the appearance of successive leaves on the main stem of wheat. It is determined by base phyllochron, leaf stage factor, and photoperiod effect.

The [Haun Stage](HaunStage.qmd) is defined as the number of leaf tips that have appeared on a cereal plant since emergence. It estimates from daily thermal time and phyllochron.

## Stages and Phases

The wheat models outputs of phase names (Phenology.CurrentPhase.Name), stage names (Phenology.CurrentStageName) and decimal stage (Phenology.Stage) are consistent with the population culm development score (PCSD) as described by @celestina_scales_2023. Observations recorded following the protocole outlined by @celestina_scales_2023 are suitable for testing model accuracy and calibrating cultivar coefficients.
...
In addition to reportable variables defined in all sub-models above, the phenology model reports several [other variables](Variables.qmd) that provide insights into the development of the wheat crop.